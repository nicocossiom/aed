...
public class BancoFiel implements ClienteBanco, GestorBanco {
	...
	public void hacerTransferencia(String idFrom, String idTo, int cantidad) throws CuentaNoExisteExc, InsuficienteSaldoExc {
		...
		else throw new CuentaNoExisteExc();
	...
	private IndexedList<Cuenta> cuentasDNI (String dni){
		...
		for (int i=0; i<cuentas.size() && !stop; i++) {
			if (dni.equals(cuentas.get(i).getDNI())) { //if we find the first element the rest must follow right behind
				for (int j=i; !stop ;j++) {
					//We will only check if it's not an element we're looking for, if it isn't it means we have found all and we can stop searching
