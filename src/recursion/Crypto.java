...
public class Crypto {
	...
	private static void encrypt(PositionList<Character> alphabet, PositionList<Character> text,
			Position<Character> calp, Position<Character> ctext, int counter,
			PositionList<Integer> encriptado) {
	...
	private static void decrypt (PositionList<Character> alphabet, 
			PositionList<Integer> encodedText, Position<Character> calp, 
			Position<Integer> cenc,int counter,
			PositionList<Character> resultdecrypt){
		...
		if(cenc==null) { //caso base=recursion termina cuando salgo de encoded 
			...
			if(counter==0) { //char siguiente encontrado en alpha
				...
				if(encodedText.next(cenc)!=null) { //recursion si no me salgo de encoded
					decrypt(alphabet, encodedText, calp, encodedText.next(cenc), encodedText.next(cenc).element(),resultdecrypt);
				}
			}
	...
	public static PositionList<Character> decrypt(PositionList<Character> alphabet, PositionList<Integer> encodedText) {
		...
		decrypt(alphabet, encodedText, alphabet.first(), encodedText.first(), encodedText.first().element(), resultdecrypt);
