...
public class Explorador {
	...
	private static void doralaExploradora(Lugar nodoActual ,Pair<Object, PositionList<Lugar>> thisIstheWay) {
		//caso base, lo que busco es el tesoro, si lo encuentro, guardo mi posicion y el tesoro,
		if(nodoActual.tieneTesoro()) {
			thisIstheWay.setLeft(nodoActual.getTesoro());
			thisIstheWay.getRight().addLast(nodoActual);
			return; //he terminado
		}
		//caso base, si estoy en un sitio que no he estado antes, lo marco y lo añado a mi lista
		...
		while(ways.hasNext()) { //recorro/dora los caminos caminos que hay desde donde estoy/dora
			...
			if(!nextnode.sueloMarcadoConTiza()) { //si el siguiente camino no lo he visitado que lo explora dora, me dira si hay un tesoro
				doralaExploradora(nextnode, thisIstheWay);
				return; //los recorrere todos a no ser que me encuentre el tesoro, dora lo ha encontrado, he terminado
			}
		}
		//dora ha recorrido todos los caminos, y si no ha parado de recorrerlos significa que no lo ha encontrado
		...
		doralaExploradora(thisIstheWay.getRight().last().element(), thisIstheWay); //que dora pruebe con los caminos que le quedaban del lugar anterior
		return;  //los recorrere todos a no se que me encuentre el tesoro, si se ejecuta esta linea es que dora lo ha encontrado, he terminado
	}
